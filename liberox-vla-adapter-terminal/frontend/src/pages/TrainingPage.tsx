      ...
      <section className="surface training-config">
        ...
        <div className="training-form">
          ...
          <details><summary>高级 IQL 参数</summary><div className="parameter-grid advanced-parameters">
            <label className="training-toggle"><input type="checkbox" checked={Boolean(parameters.reward_rynnvalue)} onChange={(event) => patchParameter("reward_rynnvalue", event.target.checked)} />启用 RynnValue Shape Reward</label>
            ...
            <label>Shape reward 系数 κ<input type="number" min={0} step="any" disabled={!Boolean(parameters.reward_rynnvalue)} value={String(parameters.reward_shaping_weight ?? "")} onChange={(event) => patchParameter("reward_shaping_weight", Number(event.target.value))} /></label>
          ...
          </div><p className="field-hint">关闭 RynnValue 时 Final Reward 只使用环境 sparse reward；以上奖励设置只重建快速 reward cache，不会再次运行模型评价。</p></details>
