...
2. 点击“创建训练数据集”，选择随机、按时间顺序、分类配额或手动勾选。预览会给出 M、预计 action/chunk 数和分类构成；确认后生成不可变、单任务数据集。修改成员必须使用“派生版本”，不会覆盖旧版本。未标注版本可“取消冻结”，已结束标注的版本可“删除数据集”；存在活动任务、派生子版本或训练记录时会拒绝删除。删除只清理该数据集清单和专属标注目录，不删除源轨迹或全局共享奖励缓存。
