...
6. 侧栏“训练”选择数据集和 Reward 来源（Sparse、RynnValue、Stage-based），可点击“配置数据集评价”原地展开与数据集页相同的表单。γ 和 cumulative reward 可为本次训练独立调整；p、κ 仅在数据集评价配置中设置。数据集缺少所选类型时，所有成员必须有完整、有效的同类型全局结果，否则列出缺失或损坏成员并禁止启动。全局回退在训练开始时复制为私有快照；各成员原有 p、κ、模型输出与完整时间轴保持独立。修改 γ/cumulative 只从已保存信号计算训练私有奖励，既不重新评价模型也不改写原结果。所有训练样本使用同一训练 γ/cumulative；不同成员保留各自的语义奖励设置。过旧 sidecar 若没有可验证的 chunk 元数据会明确报错，可在数据集配置中复用已有模型输出重建。独立 CLI YAML 与 IQL 更新公式不变。
