...
## 2026-09-15：训练页误报 Run not found

- 问题：部署设备选择旧评价的数据集时，训练配置加载失败，提示 `Error: Run not found`。
- 原因：旧 `annotation_history` 被兼容转换为评价摘要，但没有新格式的 `config_path`；训练页默认参数读取直接索引该字段，产生 `KeyError`，被统一翻译成轨迹不存在。另一条路径中，缺失成员已被记录为不可用，却在后台验证调度前被再次查找并抛错。
- 修复：旧评价、缺失评价配置返回明确的不可训练原因，页面仍可配置数据集或切换奖励；成员不存在时保留具体 ID 和错误，不启动验证。训练接口的其他字段异常保留字段名和服务器堆栈，不再伪装为 `Run not found`。
- 部署：更新代码并重新构建前端后重启 UI 服务；已有数据、评价和关键帧不变。若提示旧评价缺少训练快照，在数据集配置中保持原模型参数并关闭强制模型评价，再生成同类型结果，可复用兼容模型输出。若提示路径或轨迹缺失，先检查部署的数据目录，不要删除数据库或重新采集。
- 验证：覆盖旧评价、配置文件缺失、成员索引缺失、API 字段错误及切换奖励后清除旧提示的回归测试。
